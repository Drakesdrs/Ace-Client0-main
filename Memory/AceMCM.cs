...

{
    public class AceMCM
    {
        ...
        public static void WriteVec3<T>(ulong address, Vec3<T> value) where T : unmanaged //9
        {
            Write(address + 0, value.x);
            Write(address + 4, value.y);
            Write(address + 8, value.z);
        }

        public static void WriteVec2<T>(ulong address, Vec2<T> value) where T : unmanaged //9
        {
            Write(address + 0, value.x);
            Write(address + 4, value.y);
        }
        ...
        public static Vec3<T> ReadVec3<T>(ulong address) where T : unmanaged //17
        {
            return new Vec3<T> { x = Read<T>(address + 0), y = Read<T>(address + 4), z = Read<T>(address + 8) };
        }

        public static Vec2<T> ReadVec2<T>(ulong address) where T : unmanaged//17
        {
            return new Vec2<T> { x = Read<T>(address + 0), y = Read<T>(address + 4) };
        }
